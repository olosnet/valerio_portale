...
use crate::{
    common::helpers::secs_to_dms_string,
    oggetti_astronomici::{
...
impl OggettiAstronomiciRepository {
    ...
    pub async fn search(&self, term: &str) -> Result<Vec<OggettoAstronomico>, CornettiError> {
        let normalized_term = term.trim().to_uppercase();
        let filter = if normalized_term.is_empty() {
            doc! {}
        } else {
            doc! { "cataloghi.extended": { "$regex": format!("^{}", normalized_term) } }
        };
