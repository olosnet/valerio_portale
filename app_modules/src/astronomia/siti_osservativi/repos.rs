...
use cornetti::{
    ...
    mongo::{
        ...
        traits::{MongoBaseModel, TryMergeFrom},
    },
};
...
impl TryMergeFrom<SitoOsservativoUpdate> for MongoSitoOsservativoModel {
    fn try_merge_from(&mut self, update: &SitoOsservativoUpdate) -> CornettiResult<()> {
        ...
        self.timezone = update.timezone.clone();
        self.touch();
        Ok(())
    }
}
...
impl SitiOsservativiRepository {
    ...
    pub async fn list(&self) -> Result<Vec<SitoOsservativo>, CornettiError> {
        ...
        let items: Vec<MongoSitoOsservativoModel> = cursor.try_collect().await.map_err(|e| {
    ...
    pub async fn get(&self, sito_id: &str) -> Result<SitoOsservativo, CornettiError> {
        let obj_id =
            ObjectId::parse_str(sito_id).map_err(|_| errors::bad_request::invalid_object_id())?;
